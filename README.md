...
- Tables:
  ...
  - Syntax notes:
    - Most HTML tags can be rendered as HTML within table cells, such as:
      - `<center></center>` for centering individual cells
      - `<pre><code></code></pre>` for code blocks
    - (Custom) Insert any inline tag like `<span>` with attribute `data-col-width="[something]%"` inside any cell to control width for its column
