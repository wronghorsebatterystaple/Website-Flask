...
# For final regex etc. that Markdown extensions can't catch due to placeholders or other contraints
# (note that changes that are much easier with JQuery are left to a third round in the display_customization.js files)
def additional_markdown(s) -> str:
    # remove extra <p> tags generated by the 3rd-party extension "markdown_grid_tables" that mess up table spacing
    s = s.replace("</center></p>", "</center>").replace("</pre></p>", "</pre>")
    # using regex to not take chances here
...
def post(post_sanitized_title):
    ...
    if request.method == "GET":
        ...
        post.content = additional_markdown(post.content)
        ...
        for comment in comments:
            ...
            comment.content = additional_markdown(comment.content)
