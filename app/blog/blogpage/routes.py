...
# (note that changes that are much easier with JQuery are left to a third round in post_display_customization.js)
def additional_markdown(s) -> str:
    ...
    s = s.replace("<p><center>", "<center>").replace("</center></p>", "</center>") \
             .replace("<p><pre>", "<pre>").replace("</pre></p>", "</pre>")
